...
def get_sp_feature(args, loader, model, current_growsp):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(loader):
            ...
            if feats.dim() > 2:
            # 找出大小为0的维度索引
                zero_dims = [i for i in range(feats.dim()) if feats.shape[i] == 0]
                if zero_dims:
                # 移除第一个大小为0的维度
                    feats = feats.squeeze(zero_dims[0])
            ...
            ###
            # print(f"region_corr shape: {region_corr.shape}")
            ...
            if current_growsp is not None:
                ...
                # print("region_feats1:",region_feats.shape)
                ...
                # sp_idx = torch.from_numpy(KMeans(n_clusters=n_segments, n_init=5, random_state=0).fit_predict(region_feats.cpu().numpy())).long()
            else:
                feats = region_feats
        ...
        for batch_idx, data in enumerate(loader):
            ...
            if current_growsp is not None:
                ...
                if region_feats.size(0) < current_growsp:
                    ...
                    n_segments = current_growsp
                # print("region_feats2:",region_feats.shape)
                sp_idx = torch.from_numpy(KMeans(n_clusters=n_segments, n_init=5, random_state=0, n_jobs=5).fit_predict(region_feats.cpu().numpy())).long()
            else:
                feats = region_feats
                sp_idx = torch.tensor(range(region_feats.size(0)))
