...
def parse_args():
    ...
    parser.add_argument(
        "--data_path", type=str, default="data/S3DIS/input", help="pont cloud data path"
    )
    parser.add_argument(
        "--sp_path",
        type=str,
        default="data/S3DIS/initial_superpoints",
        help="initial superpoint path",
    )
    parser.add_argument(
        "--save_path", type=str, default="trained_models/S3DIS/", help="model savepath"
    )
    ###
    parser.add_argument(
    ...
    ####
    parser.add_argument(
        "--workers", type=int, default=10, help="how many workers for loading data"
    )
    parser.add_argument(
        "--cluster_workers",
        type=int,
    ...
    for data in test_loader:
        with torch.no_grad():
            ...
            if use_sp:
                ...
                region_scores = F.linear(
                    F.normalize(region_feats), F.normalize(classifier.weight)
                )
                region_no = 0
                for id in region_inds:
                    if id != -1:
                        valid_mask = id == region
                        preds[valid_mask] = torch.argmax(region_scores, dim=1).cpu()[
                            region_no
                        ]
                        region_no += 1
            else:
            ...
            all_preds.append(preds[labels != args.ignore_label]), all_label.append(
                labels[[labels != args.ignore_label]]
            )
...
def eval(epoch, args, test_areas=["Area_5"]):
    ...
    model = MS_SA_Deep3DBackbone_1().cuda()
    ...
    cluster_pred = KMeans(
        n_clusters=args.semantic_class, n_init=10, random_state=0, n_jobs=10
    ).fit_predict(
        primitive_centers.cpu().numpy()
    )  # .astype(np.float64))
    ...
    classifier = get_fixclassifier(
        ...
        centroids_num=args.semantic_class,
        centroids=centroids,
    ).cuda()
    classifier.eval()
    ...
    histogram = np.bincount(
        sem_num * all_labels[mask] + all_preds[mask], minlength=sem_num**2
    ).reshape(sem_num, sem_num)
    """Hungarian Matching"""
    m = linear_assignment(histogram.max() - histogram)
    o_Acc = histogram[m[:, 0], m[:, 1]].sum() / histogram.sum() * 100.0
